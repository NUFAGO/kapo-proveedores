...
class AuthService {
  ...
  async login(usuario: string, contrasenna: string): Promise<LoginResponse> {
    const query = `
      mutation LoginAdmin($usuario: String!, $contrasenna: String!) {
        loginAdmin(usuario: $usuario, contrasenna: $contrasenna) {
          id
          token
          usuario
          nombresA
    ...
    try {
      console.log('🔑 Login - Iniciando petición:', {
        usuario,
        endpoint: API_URL,
      ...
      const loginData = data.data.loginAdmin;
      ...
      if (typeof window !== 'undefined') {
        ...
        console.log('- Token length:', adaptedResponse.token.length);
        console.log('- Token preview:', adaptedResponse.token.substring(0, 50) + '...');
        
        setAuthToken(adaptedResponse.token, TOKEN_EXPIRY_DAYS);

        // Verificar que se guardó correctamente
        const savedToken = this.getCookieValue('auth_token');
        console.log('- Token guardado en cookie:', !!savedToken);
        console.log('- Token guardado preview:', savedToken ? savedToken.substring(0, 50) + '...' : 'null');
  ...
  logout(): void {
    ...
    console.log('🔒 Logout - Antes de limpiar:');
    console.log('- Cookie auth_token:', this.getCookieValue('auth_token'));
    ...
    // Logging después de limpiar
    console.log('🔒 Logout - Después de limpiar:');
    console.log('- Cookie auth_token:', this.getCookieValue('auth_token'));
    console.log('- localStorage user:', localStorage.getItem('user'));
    console.log('- localStorage refresh_token:', localStorage.getItem('refresh_token'));
  }
  ...
  /**
   * Helper para obtener valor de cookie directamente
   */
